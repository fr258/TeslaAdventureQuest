...
producer.send(new ProducerRecord<String, String>(topic,Integer.toString(value++), response.getData().toString()));	
producer.flush();
      ...
      String p = response.getData().toString();
      int start = p.indexOf("latitude");
        String latitude = p.substring(start+9, start+26);
        int start2 = p.indexOf("longitude");
        String longitude = p.substring(start2+10, start2+29);
      String jsonResposnse = "{\"latitude\":" + latitude + ", \"longitude\":" + longitude + "}";
      System.out.println(jsonResposnse);
      //producer.send(new ProducerRecord<String, String>(topic, Integer.toString(value++), response.getData().toString().substring(37)));	
      producer.send(new ProducerRecord<String, String>(topic, Integer.toString(value++), jsonResposnse));	
